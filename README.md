...
- **Saved** keeps links on this device without keeping the tabs open. Save any web tab in the window (pinned and grouped tabs too), either as a one-off reference under **Saved for later**, optionally labelled with a project, or as a reusable **Quick return** favorite, which is ordered by how often you open it. You can edit a saved link's title, project or Quick return choice, remove it, and **Open** it in a new tab in the current window. Saving never closes, groups or moves a tab. The full address is kept, including query and fragment, so `?id=1` and `?id=2` are separate links; saving the same address again keeps the existing record. Removing the extension deletes saved links, so **Export saved links (JSON)** downloads a local copy. There is no import yet. Open dashboards update each other.
...
- `tabs`: inspect open tab titles/URLs **locally** and open saved links. `tabGroups`: create/style groups. `storage`: remember the gateway URL, your keep rules, your saved links (full addresses, on this device only) and a session-only last-batch record of tab/group IDs. Saved links are never sent to Jev; the gateway still receives only selected titles and hostnames. No bookmarks or downloads permission is used.
...
Node's built-in test runner verifies protected tab eligibility, URL metadata stripping, fallback policies, response validation, and the server adapter against fixture HTTP responses. Saved links are tested with fake `chrome.storage` and `chrome.tabs` objects: exact-address deduplication, concurrent saves, failed writes, a newer read-only schema, reopen success and failure, and the JSON export round trip. Packaging uses the system `zip` command and includes **only extension files**, never server files, secrets or dependencies. Release zip is produced under `dist/`. JavaScript edits require reloading the extension in `chrome://extensions`, then reopening its dashboard.

Optional real Chromium smoke test: install Playwright separately (`npm install --no-save playwright`, `npx playwright install chromium`) and run `node scripts/browser-smoke.mjs`. If Playwright is already installed globally, point the script at it instead: `PLAYWRIGHT_MODULE=$(npm root -g)/playwright/index.mjs node scripts/browser-smoke.mjs`. It uses an isolated temporary profile and synthetic intercepted pages, checks actual MV3 grouping/ungrouping and stale-tab protection, the Tidy preview, and saved links: save/edit/remove without tab changes, two dashboards, reopen, export and persistence across a restart of that profile. Reopen checks use a loopback address that fails locally, so nothing leaves the machine. It saves `dist/browser-smoke.png` and `dist/browser-smoke-saved.png`. The runtime extension itself has no dependencies.
